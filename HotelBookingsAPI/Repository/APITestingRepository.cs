...
namespace Repository
{
    public class APITestingRepository : Repository
    {
        ...
        public async Task<bool> SeedAsync(IEnumerable<Hotel> hotels, IEnumerable<Room> rooms, IEnumerable<Booking> bookings)
        {
            try
            {
                await dbContext.Database.ExecuteSqlCommandAsync("DBCC CHECKIDENT('dbo.Hotel', RESEED, 1)");

                await dbContext.Hotels.AddRangeAsync(hotels);
                await dbContext.SaveChangesAsync();

                await dbContext.Database.ExecuteSqlCommandAsync("DBCC CHECKIDENT('dbo.Room', RESEED, 1)");
                await dbContext.Rooms.AddRangeAsync(rooms);
                await dbContext.SaveChangesAsync();

                await dbContext.Database.ExecuteSqlCommandAsync("DBCC CHECKIDENT('dbo.Booking', RESEED, 1)");
