...
namespace HotelBookingsTest
{
    public class HotelsControllerTest
    {
        ...
        [Fact]
        public async void GetAllAsync_WhenCalledWithSearchText_ReturnsMatchedNamedHotel()
        {
            // Act
            var hotels = await controller.GetAllAsync("Marriott");

            // Assert
            Assert.IsAssignableFrom<IEnumerable<Hotel>>(hotels);
            Assert.Equal(2, hotels.ToList().Count);
        }
        ...
        {
            // Act
            var result = await controller.PostAsync(new Hotel() { Name = "A Hotel", Address = "Test Address", Description = "Test Description" });
